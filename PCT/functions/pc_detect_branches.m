...
%-----------------------------------------------------------------------------------------
function [pcstemsurf,pcbranches,pcrestofcrown,branchlist]=pc_detect_branches(tree_id,pc,resultsfolder)
    ...
    if ~isfolder(strcat(resultsfolder,'/stemfigs')); mkdir(strcat(resultsfolder,'/stemfigs'));end
    saveas(gcf,strcat(resultsfolder,sprintf('/stemfigs/stemfig_%i.png',tree_id)))
    ...
    pcwrite(pcstemsurf,strcat(resultsfolder,sprintf('/pointclouds/stem/tree%i_stempoints.ply',tree_id)));
    pcwrite(pccrown,strcat(resultsfolder,sprintf('/pointclouds/canopy/tree%i_canopypoints.ply',tree_id)));
    ...
    pcwrite(pcbranches,strcat(resultsfolder,sprintf('/pointclouds/canopy/tree%i_branches.ply',tree_id)));
    pcwrite(pcrestofcrown,strcat(resultsfolder,sprintf('/pointclouds/canopy/tree%i_restofcrown.ply',tree_id)));
    
    savedir = strcat(resultsfolder,'/branchlists');
    if ~isfolder(savedir);mkdir(savedir);end
    writetable(branchlist,strcat(savedir,sprintf('/tree%i_branchlist.csv',tree_id)))
    ...
    try
        ...
        savedir = strcat(resultsfolder,'/figures/branchclassif');
        if ~isfolder(savedir);mkdir(savedir);end
        saveas(gcf,strcat(savedir,sprintf('/tree%i_branchclassif.png',tree_id)))
    catch
