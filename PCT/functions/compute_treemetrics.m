...
function treemetr = compute_treemetrics(tree_id,rootResults,h_butt,h_interval,h_layer,min_points,p)
    ...
    treemetr.Properties.VariableNames = {'tree_id','X','Y','dbh','ba','h','vol','d6','d05','vlog','vpulp'};
    treemetr.tree_id = tree_id;
    ...
    savedir = sprintf(strcat(rootResults,'/%i/stemcurves_raw'),plot_id);
    if isfolder(savedir) == false, mkdir(savedir);end
    writetable(array2table(dhlist,'VariableNames',{'h','d'}),strcat(savedir,sprintf('/%i_tree%i_stemcurve_raw.txt',plot_id,tree_id)),'Delimiter','\t');
    
    if size(dhlist,1) > 5
        sampleSize = 6; % number of points to sample per trial
        maxDistance = 0.0005; % max allowable distance for inliers
        
        dhlist = rmoutliers(dhlist,1);
        points = dhlist(dhlist(:,1)>0.2*h_end,:);
        fitLineFcn = @(points) polyfit(points(:,1),points(:,2),2); % fit function using polyfit
        ...
        for ii = 1:size(interv,1)-1
            interv(ii,3) = interv(ii+1,1) - interv(ii,1);
        end
        maxhh = min(interv(interv(:,3) > 0.75,1));

        if ~isempty(maxhh);inlierPts = inlierPts(inlierPts(:,1)<=maxhh,:);end

        dhlist = cat(1,dhlist(dhlist(:,1)<=0.2*h_end,:),inlierPts);
        treemetr.hstemmax = max(inlierPts(:,1));

        savedir = sprintf(strcat(rootResults,'/%i/stemcurves_raw_olrm'),plot_id);
        if isfolder(savedir) == false, mkdir(savedir);end
        writetable(array2table(dhlist,'VariableNames',{'h','d'}),strcat(savedir,sprintf('/%i_tree%i_stemcurve_raw_olrm.txt',plot_id,tree_id)),'Delimiter','\t');
    else
